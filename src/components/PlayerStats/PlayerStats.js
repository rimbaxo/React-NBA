...
function PlayerStats(props) {
    ...
    const currentPlayer = playersListData.filter((player) => parseInt(player.personId) === id)[0];

    const teamImage = teamList.filter((team) => team.teamId === parseInt(currentPlayer.teamId))[0];

    // array of all id's. It will be useful for scrolling through the players
    const ids = playersListData.filter((player) => player.isActive === true).map(function(player){return parseInt(player.personId);});
            ...
            <div className="row">
                ...
                <div className="col">
                    ...
                    <div className={style.navigation}>
                        ...
                        {ids.indexOf(id) !==0 &&
                        <NavLink className={`${style.navItem}`} to={`/players/${find_prev_id(id,ids)}`}> &lt; </NavLink>
                        }

                        <h1>{currentPlayer.firstName} {currentPlayer.lastName}</h1>

                        {ids.indexOf(id)!==ids.length-1 &&
                        <NavLink className={`${style.navItem}`} to={`/players/${find_next_id(id,ids)}`}> &gt; </NavLink>
                        }
                    </div>
                </div>
            </div>
